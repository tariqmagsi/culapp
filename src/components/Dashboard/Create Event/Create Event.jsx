...
export default class CreateEvent extends Component {
    ...
    validateForm = () => {
        return this.state.name.length > 0 && this.state.category.length > 0 && this.state.description.length > 0 && this.state.address.length > 0 && this.state.start_time.length > 0 && this.state.end_time.length > 0 && this.state.visibility_scope.length > 0;
    }
    ...
    fetchUsers = (query) => {
        ...
        .then(res => {this.setState({invited_guest_usernames: res.data.results ? res.data.results.length > 0 ? res.data.results.map((item,i) => ({id: i+1, name: item.username})) : [] : res.data, isLoading: false, error: ""})})
        .catch(err => {
            this.setState({isLoading: false})
        })
    }
        ...
        return (
            <div>
                ...
                <div style={{marginLeft: window.innerWidth > 600 ? (this.props.isOpened ? "200px" : "55px") : "55px",  color: "black", paddingTop: "50px", paddingLeft: "10px", overflowX: "auto"}}>
                    <h3 style={{color, marginLeft: "20px"}}>Create Event</h3>
